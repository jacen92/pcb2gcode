...
"""
    Used to create a debian package with all dependancies
"""
...
def create_control_file(dest, info, embed_deps=False):
    """
        ...
        if embed_deps == False then add dependancies data
    ...
    with open(os.path.join(fdest, "control"),'w') as f:
        ...
        if not embed_deps and info.get("dependancies"):
            f.write("Depends: %s\n" % info.get("dependancies"))
        ...
        f.write("Homepage: jacen92.free.fr\n" )
